...
def savepng(kospi, kospi200, kosdaq, thema):
    ...
    totaltxt = ''' 위에서 언급한 네 가지의 방법으로 구해진 각각의 유망한 종목들의 총점을 구한 결과'''
    predicttxt = '''위에서 구한 유망 종목의 지난 주가를 회귀분석을 사용하여 이틀 뒤의 주가를 예측한 값'''
    ...
    drawBox(ws,1,84,7,101)
    
    img = Image(path + "/" + "img.png")
    ws.add_image(img,"A"+str(84))
    
    ws.merge_cells(start_column = 1, end_column = 7, start_row = 102,end_row = 102)
    ws.cell(column = 1, row = 102).value = predicttxt
    drawBorder(ws,1,102,7,102)
